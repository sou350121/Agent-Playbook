...
1.  **可观测性底线 (Observability)**：禁止写没有日志（Logging）和链路追踪（Tracing）的逻辑。任何失败路径必须可追溯、可告警。
2.  **安全性闭环 (Security)**：输入校验、SQL 注入防御、敏感信息脱敏是默认动作，无需指令重复。
3.  **并发与异步安全**：在处理高并发或异步任务时，必须考虑幂等性（Idempotency）、重试机制（Retry）与背压管理（Backpressure）。
